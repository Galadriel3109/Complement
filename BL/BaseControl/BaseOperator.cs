...
namespace BL.BaseControl
{
    public class BaseOperator
    {
         ...
         */
        public static async Task<ML.Result> MultiConfirmation(ML.Operator.RouteHeader route,string user, string pto_alm, string mode)
        {
            ML.Result result = new ML.Result();
            try
            {
                ...
                foreach (ML.BaseControl.Confirmation confirmation  in confirmationList)
                {
                    ...
                    //GetData
                    ML.Result resultGetDetails = await GetDetails(confirmation.OrdRel, route.PtoAlm, mode, 1);
                ...
                foreach(ML.BaseControl.Interface inter in  interList)
                {
                    ...
                    {
                        throw new Exception($@"{resultCreateFile.Message}");
                    }
                    Thread.Sleep(500);
                }
        ...
        private static async Task<ML.Result> GetDetails(string ordRel, string pto_alm, string mode)
        {
            ML.Result result = new ML.Result();
            try
            {
                using (HttpClient client = new HttpClient())
                {
                    ...
                    Uri uri = new Uri($@"{DL.ApiOracle.GetEndpoint(pto_alm,mode)}".Replace("{########}", ordRel));
        ...
        private static async Task<ML.Result> GetDetails(string ordRel, string pto_alm, string mode, int vez)
        {
            ML.Result result = new ML.Result();
            try
            {
                if(vez < 5)
                {
                    ...
                    using (HttpClient client = new HttpClient())
                    {
                        ...
                        {
                            throw new Exception($@"Se obtuvo un error al consultar los detalles de envio WMS no responde {ordRel}");
                        }
                    }
                }
                else
            ...
            catch (Exception ex)
            {
                ...
                if (vez < 5)
                {
                    ML.Result resultGetDetails = await GetDetails(ordRel, pto_alm, mode, vez + 1);
                    result = resultGetDetails;
                }
                else
                {
                    result.Correct = false;
                    result.Message = $@"Se obtuvo un error al consultar los detalles de envio en WMS {ex.Message}.";
                    result.Ex = ex;
                }
            }
            return result;
