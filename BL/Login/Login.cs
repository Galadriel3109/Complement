...
namespace BL.Login
{
    public class Login
    {
        ...
        private static ML.Result ValidatePasswordLGA(string userid, string password, string mode)
        {
            ...
            try
            {
                string query = $@"SELECT TRIM(B.usu_pass), TRIM(B.cv_area), TRIM(B.usu_nombre), A.sub_rol, TRIM(A.almacenes)
                                FROM ora_lga_usu A, dblga@lga_prod:lgausuario B
                                WHERE A.usu_id = ?
                                AND B.usu_id = A.usu_id";
                        ...
                        using (OdbcDataReader reader = cmd.ExecuteReader())
                        {
                            if (reader.Read())
                            {
                                ...
                                if (match)  
                                {
                                    ...
                                    logged.pto_alm = reader.GetString(4).Split(',')[0];
                                    logged.almac = reader.GetString(4);
        ...
        private static ML.Result ValidatePasswordOPE(string userid, string password, string mode)
        {
            ...
            try
            {
                string query = $@"SELECT TRIM(password), TRIM(nom_ope), cen_pto
                                FROM ora_operadores
                                WHERE rfc_ope = ?
                                AND active = '1'";
                        ...
                        using (OdbcDataReader reader = cmd.ExecuteReader())
                        {
                            if (reader.Read())
                            {
                                ...
                                if (match)  
                                {
                                    ...
                                    logged.nombre = logged.usu_nombre.Split(' ')[0];
                                    logged.pto_alm = reader.GetString(2); 
                                    logged.almac = reader.GetString(2);
