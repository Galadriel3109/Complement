...
namespace BL.DeliveryPlanner
{
    public class DeliveryPlanner
    {
        ...
        public static ML.Result GetReadyOrdersPerDate(ML.DeliveryPlanner.ReadyQuery readyQuery, string mode)
        {
            ...
            try
            {
                ML.Result resultGetOracleCode = GetOracleCode(readyQuery.PtoAlm, mode);
                ...
                {
                    ML.DeliveryPlanner.ApiRequestWMS apiRequestWMS = GetOracleWMSInfo(readyInfo.OrdRel, facility, 1, mode);
        ...
        private static ML.DeliveryPlanner.ApiRequestWMS GetOracleWMSInfo(string orderNumber, string facility, int numTry, string mode)
        {
            ...
            if (numTry <= 3)
            {
                ...
                try
                {
                    using (HttpClient client = new HttpClient())
                    {
                        ...
                        string url = DL.ApiOracle.GetEndpointOrder(mode).Replace("{########}", orderNumber).Replace("{???}", facility);
                        Uri uri = new Uri(url);
                        ...
                        {
                            //Recursivo
                            result = GetOracleWMSInfo(orderNumber, facility, numTry + 1, mode);
                        }
                    }
                }
                catch (Exception ex)
        ...
        private static ML.Result GetOracleCode(string cod_pto, string mode)
        {
            ...
            {
                result.Correct = false;
                result.Message = $"Error al obtener el origin code de {cod_pto}: {ex.Message} ";
                result.Ex = ex;
            }
            return result;
        }
        ...
        public static ML.Result GetOrdersPerDate(ML.DeliveryPlanner.PlanQuery planQuery, string mode)
        {
            ...
            try
            {
                ML.Result resultGetOracleCode = GetOracleCode(planQuery.PtoAlm, mode);
                if (!resultGetOracleCode.Correct)
                {
                    throw new Exception($@"{resultGetOracleCode.Message}");
                }
                ...
                {
                    ML.DeliveryPlanner.ApiRequestWMS apiRequestWMS = GetOracleWMSInfo(planInfo.OrdRel, facility, 1, mode);
