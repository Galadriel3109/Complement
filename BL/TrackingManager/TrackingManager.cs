            ...
            try
            {
                using(OdbcConnection connection = new OdbcConnection(DL.Connection.GetConnectionStringGen(mode)))
                {
                    ...
                    string query = $@"SELECT
                                        ...
                                        CASE
	                                        WHEN B.estado = 'P' THEN '-Retenido o Transito'				
	                                        ...
	                                        WHEN B.estado = 'G' THEN '-Generado Backorder'
	                                        WHEN B.estado = 'W' THEN '-Cerrado Por Inventario'
                                        ...
                                        CASE
	                                        ...
	                                        WHEN D.rt_stat = 5 THEN 'Evento sin registrar cerrado por inventario'
	                                        WHEN D.rt_stat = 6 THEN 'Retencion no concluida cerrado por inventario'
	                                        ELSE 'Estado No listado.'
                                        END AS estado_rt,
	                                    CASE
